...
def upload_file(task_id=None):
    ...
    task = run_training.delay(
        app.config["DATASET"], filepath, app.config["DOWNLOAD_FOLDER"], filename
    )
...
def run_training(self, dataset, filepath, model_dir, image):
    ...
    def log_progress(
        ...
        self.update_state(
            meta={
                "current": count + epoch * num_images,
                "total": num_images * num_epochs,
                "status": message,
    ...
    if app.config["CONVERT_TO_OPEN_VINO"]:
        ...
        with ZipFile(
            os.path.join(app.config["DOWNLOAD_FOLDER"], archive), "w"
        ) as zip_file:
            zip_file.write(pytorch_model, model)
            vino_xml = model_name + ".xml"
            zip_file.write(
                os.path.join(app.config["DOWNLOAD_FOLDER"], vino_xml), vino_xml
            )
            vino_bin = model_name + ".bin"
            zip_file.write(
                os.path.join(app.config["DOWNLOAD_FOLDER"], vino_bin), vino_bin
            )
            zip_file.close()
        to_return = archive
    else:
