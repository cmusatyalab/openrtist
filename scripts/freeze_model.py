...
class Tracer():
    ...
    def freeze(self, weight_file_path="models_1p0/starry-night.model",
    ...
    def quantize_and_freeze(self, weight_file_path="models_1p0/starry-night.model",
        ...
        """
        self._model.load_state_dict(torch.load(weight_file_path))
        self._model = torch.quantization.quantize_dynamic(self._model,
                                                          dtype=torch.qint8)
        model_input = torch.rand(1, 3, 224, 224)
        traced_model = torch.jit.trace(self._model, model_input)
        traced_model.save(output_file_path)
